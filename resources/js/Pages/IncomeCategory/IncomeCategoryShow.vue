...
export default {
    ...
    methods: {
        ...
        submit() {
            this.form.put(route('incomeCategories.update',this.incomeCategory['id']),{
                onSuccess: () => {
                    const $toast = useToast();
                    let instance = $toast.success('You have successfully updated a income category!');
                    this.isEdit= false;
                    this.form.reset();
                },
                onError: () => {
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between">
                <div>
                    <h2 class="font-semibold text-2xl text-gray-800 leading-tight">{{ incomeCategory['name'] }}</h2>
                </div>
                <div>
